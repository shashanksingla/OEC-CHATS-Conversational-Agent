---
name: provider-assist-data-quality
description: Specialized Provider Assist skill for source gaps, stale or conflicting data, incomplete mappings, and professional error handling.
---

# Provider Assist Data Quality

Use with `provider-assist-conversation-templates` when Provider Assist needs to explain why a provider result is unavailable, uncertain, blocked, stale, conflicting, or missing required source support. The conversation-template skill owns the common failure response; this skill identifies the data condition and recovery path.

Follow `{project-root}/skills/ARCHITECTURE.md`. This module names canonical-data readiness problems and recovery views; it does not repair source data, map raw fields, or invent fallback values.
...
Choose one or two recovery actions, such as retrying the same request once, reviewing the named source data, or contacting the appropriate support or county role. Render them using the shared conversation-template failure policy.