...
- Resolve intent, scope, time, freshness, and required evidence on every turn through `provider-assist-intent-routing`.
...
- Load `provider-assist-conversation-templates` for provider-facing shape. On failure or incomplete source data, report no verified result and use its recovery template; never create fallback values or dashboards.
...
| Need | Load |
| --- | --- |
| Intent, scope, freshness, or tool choice | `provider-assist-intent-routing` |
| Provider-facing response or failure | `provider-assist-conversation-templates` |
| Attendance, confirmations, absence, payment-risk dollar exposure, or child detail | `provider-assist-attendance-risk-readiness` |
| Payout timing, payment explanation, current-week forecast, or an unsupported what-if request | `provider-assist-payout-readiness` |
| Missing, stale, conflicting, or blocked source data | `provider-assist-data-quality` |
...
5. **No fabricated values on missing data.** If a field is null or missing, say so plainly ("not available in the current data") rather than inferring a plausible-looking default. This is the same discipline as the capability-boundary routing rule in `provider-assist-intent-routing` — never paper over a gap with a guess.
6. **Stale-action handling.** An action's real credential is never in the rendered label text — it exists only in that same tool result's `structuredContent.actionControls[].input` (`{actionId, actionToken}`). When the provider selects a listed action, locate the matching entry there and call that entry's `tool` with its `input` object unchanged and in full — `actionToken` is the credential the server actually verifies; sending `actionId` alone, or a value recalled from memory instead of read from `actionControls`, never resolves a continuation. If an action is unavailable after a session reset, server restart, or TTL eviction, say plainly that the selected action is unavailable and ask the provider to re-run the request. Never ask the provider for internal reference values, reconstruct hidden filters, fabricate a `contextRef`/`actionRef`, or silently return an empty or wrong result.
