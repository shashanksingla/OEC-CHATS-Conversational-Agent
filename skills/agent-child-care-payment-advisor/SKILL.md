...
6. **Stale-action handling.** Treat action controls as opaque and send only their `actionId`. If an action is unavailable after a session reset, server restart, or TTL eviction, say plainly that the selected action is unavailable and ask the provider to re-run the request. Never ask the provider for internal reference values, reconstruct hidden filters, or silently return an empty or wrong result.
