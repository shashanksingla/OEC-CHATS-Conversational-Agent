...
- **Transaction Validity** — `Status__c` is the parent-confirmation source. Only `PARENT_APPROVED` or `PENDING_PROVIDER` statuses count; `PARENT_PENDING` remains pending and `PARENT_REJECTED` is excluded. Only `CI_Transaction_Result__c == 1` counts; a transaction must reference the schedule being processed; a transaction denied for no drop-in days remaining does not count and does not consume the drop-in allowance.
...
- Overnight schedules are out of scope for this module.
- Orphan transactions and orphan drop-in transactions are out of scope because attendance records must be linked to a provider schedule.
- The 36-month enrollment-absence exception is payable and is not treated as a payment blocker.
