...
The rendered Next Actions line is `{n}. {canonical label}` only — the `reason` field that accompanies each action in code stays as internal ranking/telemetry metadata and is never appended to the rendered line (no `(reason text)` suffix). Numbers restart at 1 on every response and are scoped to that turn only; they are never persisted or reused once a new response replaces the list. See `skills/provider-assist-conversation-templates/SKILL.md` for the full selection rules (by number or by typed label).