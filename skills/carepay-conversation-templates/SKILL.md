...
For a `riskFocus: "ABSENCE_LIMITS"` drill-down specifically, use `| Child | Authorization | County | Absences used | County limit |`, where `Absences used` is the child's `absence_days` count and `County limit` is that child's `absence_limit`. Do not add an `Over limit` column: the provider can compare the two values directly, and the shared explanation can state any excess once above the table. For parent-confirmation and incomplete-attendance drill-downs, keep authorization in the child-level table and use only columns that help resolve that specific issue.
