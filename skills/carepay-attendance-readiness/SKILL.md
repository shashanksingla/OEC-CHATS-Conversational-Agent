...
For a request naming one or more counties instead of a child (`show children for Adams and Denver`, `who is affected in Denver`), pass those exact county names as `countyNames` to `cccap_analyze_attendance_risk` alongside the currently active scope and `riskFocus` (if any). This narrows the already-returned child-level rows to the named counties; it does not require a new source read and works whether the current result was freshly retrieved or reused from a preceding continuation, because every returned child row already carries its county. Do not respond that county-level child detail is unavailable, and do not fall back to a county-only aggregate table, when the analysis result already contains child rows — the `countyNames` filter is exactly how a facility-wide result narrows to a named county's children. If a named county returns no affected children, say so by name rather than describing the drill-down as unsupported.
