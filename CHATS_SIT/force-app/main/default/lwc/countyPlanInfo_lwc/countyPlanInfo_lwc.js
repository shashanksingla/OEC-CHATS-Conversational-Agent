...
import { getQuestionByKey, getSectionsByCategory, getHeaderTitleByKey, evaluateFormulaByApiName } from 'c/countyPlanQuestionsUtility';

export default class CountyPlanInfo_lwc extends LightningElement {
    @api compareList;
    @api overAllFeedback = {};
    @track
    columns = [{
        ...
        try {
            ...
            this.compareList.forEach((val, index) => {
                ...
                if (val.Status__c == 'Conditionally Approved') {
                    ...
                    dateToShow = this.formatDateToMMDDYYYY(val.Action_Date__c || val.DTE_BEGIN_EFFEV__c);
                } else if (val.Status__c == 'Approved') {
                    statusLabel = 'Approved';
                    dateToShow = this.formatDateToMMDDYYYY(val.DTE_BEGIN_EFFEV__c);
                } else {
                ...
                if (this.hasPriorSubmissions) {
                    label += ' ' + dateToShow;
                    if (val.DTE_BEGIN_EFFEV__c && index > 0) {
                        label += '\n' + val.Status__c + ' (' + this.formatDateToMMDDYYYY(val.DTE_BEGIN_EFFEV__c) + ')';
                    }
                }
                columns.push({
            ...
            console.log('Error in connectedCallback: ' + err.message);
        }
    ...
    get tables() {
        let tableData = [];
        let categories = getSectionsByCategory('CP');
