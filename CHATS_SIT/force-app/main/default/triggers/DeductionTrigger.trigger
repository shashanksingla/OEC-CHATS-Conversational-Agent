...
*********************************************************************************************/
trigger DeductionTrigger on T_INDIV_DDTN__c (after insert, after update) {
    //Check trigger active on/off
    if(TriggersOnOffServices.triggerStatus('DeductionTrigger')) {
    ...
    if(Trigger.isAfter) {
        ...
        if(Trigger.isInsert) {
            ...
            CopySalesforceId.stampSalesforceIdToExternalId([select Id,IDN_EXTNL__c from T_INDIV_DDTN__c where Id=:trigger.newMap.keySet()],true);
        }
    }
