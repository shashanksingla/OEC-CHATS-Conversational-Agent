({
    ...
    getParams: function(component, event, helper) {
        ...
        var secondarySearchField = component.get('v.secondarySearchField');
        var subtitleField = component.get('v.subtitleField');
        var alternateSearchField = component.get('v.alternateSearchField');
        return {
            filter: filter,
            limit: limit,
            object: object,
            ...
            secondarySearchField: secondarySearchField,
            subtitleField: subtitleField,
            alternateSearchField: alternateSearchField
        };
    },
    getRecentRecords: function(component, event, helper) {
        ...
        getRecordsAction.setCallback(this, function(res) {
            if (res.getState() === 'SUCCESS') {
                ...
                if (!$A.util.isEmpty(returnValue) && returnValue.isSuccess && Array.isArray(returnValue.results.data)) {
    ...
    getRecordByValue: function(component, event, helper) {
        ...
        getRecordsAction.setCallback(this, function(res) {
            if (res.getState() === 'SUCCESS') {
                ...
                if (!$A.util.isEmpty(returnValue) && returnValue.isSuccess) {
                    returnValue.results.data.forEach(function(record) {
    ...
    getRecordsBySearchTerm: function(component, event, helper) {
        ...
        //CHATS-5303 Bug fix
        if(component.get('v.searchField') && component.get('v.searchField') === 'Service_Period_Dates__c')
            ...
            getRecordsAction.setCallback(this, function(res) {
                if (res.getState() === 'SUCCESS') {
                    var returnValue = JSON.parse(res.getReturnValue());
                     ...
                     if (!$A.util.isEmpty(returnValue) && returnValue.isSuccess && returnValue.results.searchTerm === searchTerm) {
                        var returnedRecords = [];
    ...
    addDelay: function(component, event, helper) {
        var secondsToDelay =$A.get("$Label.c.CUSTOM_LOOKUP_DELAY_TIME");
        window.setTimeout(
