        ...
        if(isValid==true){
            helper.callServerAndHandleError(component,"c.checkCountyWithOwnerCounty", 
                                            function(response){
                                                ...
                                                if(component.get("v.ownerId")!=component.get("v.caseRec.OwnerId")){
                                                    ownerChanged = true;
                                                }
                                                if(response.objectData.countyMatched==false && response.objectData.ownerCountyMatched==false){
                                                    countyMismatchMessage = 'The case that you are trying to update is with '+response.objectData.countyName+' county. This does not match your assigned county(ies)';
                                                    ...
                                                    countyMismatchMessage = 'The case that you are trying to update is with '+response.objectData.countyName+' county. This does not match the county(ies) of new owner.';
                                                    component.set("v.pageMessages",countyMismatchMessage);
                                                    component.set("v.messageType","error");
                                                }else{
                                                    helper.callServerAndHandleError(component,"c.upsertRecordsFinal", 
                                                                                    ...
                                                                                    }, {'lstSObject':[component.get("v.caseRec")],"isFinalStep":true}, false, null);
                                                }
                                            }, {'countyId':component.get("v.caseRec.CDE_COUNTY__c"), 'ownerId':component.get("v.caseRec.OwnerId")}, false, null);
        }else{
