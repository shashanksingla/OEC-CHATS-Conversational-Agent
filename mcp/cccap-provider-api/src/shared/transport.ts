...
} from "../payment/payment-engine.js";
...
// Fired only when an authorization's fiscal schedule fails to match specifically because
// no candidate schedule (matched on county) carried the requested rate type - the exact
// "rate not yet available for these dates" root cause. Includes every candidate schedule
// that WAS considered (its ID and actual rate type) so the mismatch can be traced back to
// the specific Rate_Schedule__c record(s) in Salesforce.
export interface FiscalMatchDiagnostic {
...
interface ClientOptions {
  ...
  onFiscalMatchUnresolved?: ((entry: FiscalMatchDiagnostic) => void) | undefined;
}
...
export class CccapClient {
  ...
  private readonly onFiscalMatchUnresolved: ((entry: FiscalMatchDiagnostic) => void) | undefined;
  ...
  public async getAuthorizations(
    ...
    return {
      ...
      normalizedAuthorizations: authorizations.map((authorization) => {
        ...
        const fiscalScheduleMatch = selectFiscalScheduleForAuthorization(
          authorization,
