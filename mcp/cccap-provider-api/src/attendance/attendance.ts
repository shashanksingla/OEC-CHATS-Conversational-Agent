...
import { buildSituationEnvelope, type SituationEnvelope } from "../shared/situation-envelope.js";

// Rejects sandbox/QA-labeled facility names so the greeting falls back to a generic phrase.
function isLikelyFacilityName(value: string | undefined): value is string {
...
export async function getAttendanceRiskAnalysis(
  ...
  detailPage: number | undefined;
  detailPageSize: number | undefined;
  ...
  for (const value of schedules) {
    ...
    if (!authorizationId || typeof rateType !== "string") continue;
    const set = scheduleRateTypesByKey.get(authorizationId) ?? new Set<string>();
    set.add(rateType);
