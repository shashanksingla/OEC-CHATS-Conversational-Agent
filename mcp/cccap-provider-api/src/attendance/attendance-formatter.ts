...
import { actionControls, attendanceScopeLabel, compactActionControls, estimatedMoney, hasAbsenceLimitConcern, numericValue, overLimitDays, recordValue, renderActionSections, result, shortDateLabel, tableValue, type ToolResult, MAX_DISPLAY_CHILDREN, MAX_SUMMARY_ROWS } from '../shared/formatters/shared.js';
import { actionViewMetadata, isNoOpAction, isRecentlyRenderedAction, viewState } from '../shared/formatters/shared.js';
import type { RankedNextAction } from '../shared/situation-envelope.js';

// Reasons for risk-driven actions sourced from situation.rankedActions - the ranking engine
// itself has no "reason" concept (it only scores/labels), so this maps known action IDs to the
// existing provider-facing rationale. An action_id not listed here (e.g. a dynamic
// summary_view.next_actions entry) falls back to a generic reason rather than guessing specifics.
const RANKED_ACTION_REASONS: Record<string, string> = {
...
function renderAttendanceNumericTable(headers: string[], rows: string[][], numericColumns: number[]): string[] {
  // Preserve unavailable markers because they explain unresolved values.
...
export function compactAttendanceSummary(attendanceView: Record<string, unknown> | undefined): Record<string, unknown> | undefined {
...
export function sourceIntegrityError(scope: unknown, message: string): ToolResult {
  return {
    isError: true,
    content: [{
      type: "text" as const,
...
/**
 * Single public entry point for every attendance path - the greeting/
 * facility-pulse snapshot (getAttendanceRiskSnapshot/
 * getCurrentMonthAttendanceSnapshot) and the risk-analysis drill-down
 * (getAttendanceRiskAnalysis). Dispatches purely on the shape of the raw
 * orchestration result: a top-level `providerMessage` string identifies the
 * snapshot path; otherwise the result is treated as a risk-analysis result.
 * Mirrors payment-formatter.ts's formatPayoutResult dispatcher.
 */
export function formatAttendanceResult(data: unknown, includeContinuationMetadata = false): ToolResult {
  const record = recordValue(data);
  if (record && typeof record.providerMessage === "string" && record.providerMessage.length > 0) {
    return formatAttendanceSnapshot(data, includeContinuationMetadata);
  }
  return formatAttendanceRiskAnalysis(data, includeContinuationMetadata);
}

// Internal - handles the attendance-risk analysis shape (data.attendanceRisk
// with a children array, no providerMessage). Reached only through
// formatAttendanceResult() above.
function formatAttendanceRiskAnalysis(data: unknown, includeContinuationMetadata = false): ToolResult {
  ...
  const sourceUnmatchedChildNames = Array.isArray(risk.unmatched_child_names)
    ? risk.unmatched_child_names.filter((name): name is string => typeof name === "string")
    : [];
  const excludedClosureDates = Array.isArray(risk.excluded_closure_dates)
    ? risk.excluded_closure_dates.filter((date): date is string => typeof date === "string")
    : [];
  ...
  const requestedChildNameList = Array.isArray(requestedChildNames)
    ? requestedChildNames.filter((name): name is string => typeof name === "string" && name.length > 0)
    : [];
  ...
  // Filters narrow cached results without widening scope or rereading the source.
  const childScopedChildren = normalizedRequestedChildren.size > 0
    ? riskFocusedChildren.filter((child) =>
      typeof child.child_name === "string" && normalizedRequestedChildren.has(child.child_name.trim().toLowerCase()),
    )
    : riskFocusedChildren;
  const affectedChildren = normalizedRequestedCounties.size > 0
    ? childScopedChildren.filter((child) =>
  ...
  const unmatchedChildNames = [
    ...
    ...requestedChildNameList.filter((name) =>
      !riskFocusedChildren.some((child) => typeof child.child_name === "string" && child.child_name.trim().toLowerCase() === name.trim().toLowerCase()),
    ),
  ].filter((name, index, names) => names.indexOf(name) === index);
  const unmatchedCountyNames = normalizedRequestedCounties.size > 0
    ? requestedCountyNames.filter((name) =>
      !childScopedChildren.some((child) => typeof child.county === "string" && child.county.trim().toLowerCase() === name.trim().toLowerCase()),
    )
    : [];
  ...
  // Validate aggregates only for the full, unnarrowed result.
  ...
  const attendanceView = attendanceSummary(
    risk,
    affectedChildren,
    analysis.scope,
    unmatchedChildNames,
    excludedClosureDates,
    excludedHolidayDates,
  );
  // Hoisted because the county-first branch also uses these aggregates.
  ...
  const attendanceCounties = Array.isArray(attendanceView.counties)
    ? attendanceView.counties.map(recordValue).filter((row): row is Record<string, unknown> => Boolean(row))
    : [];
  const attendanceViews = Array.isArray(attendanceView.available_views)
    ? attendanceView.available_views.map(recordValue).filter((row): row is Record<string, unknown> => Boolean(row))
    : [];
  ...
  // shortDateLabel already includes the year.
  const attendancePeriodHeaderLabel = attendancePeriodBeginLabel && attendancePeriodEndLabel
    ...
    : undefined;
  // Keep the primary interpretation visually distinct.
  ...
  const lines = [
    `**${interpretationLine}**`,
    // Keep the scope period header visually distinct.
  ...
  // Use a blockquote so the exclusion note reads as explanatory context.
  ...
  // Show holiday exclusions only on views where absence context applies.
  if (riskFocus !== "PARENT_CONFIRMATIONS" && riskFocus !== "INCOMPLETE_ATTENDANCE" && excludedHolidayDates.length > 0) {
  ...
  if (riskFocus !== "ABSENCE_LIMITS" && riskFocus !== "INCOMPLETE_ATTENDANCE" && pendingDays > 0) {
    // Emphasize the time-critical confirmation deadline.
  ...
  if (riskFocus !== "PARENT_CONFIRMATIONS" && riskFocus !== "INCOMPLETE_ATTENDANCE" && absenceChildren > 0) {
    // Separate exceeded limits from children merely approaching the limit.
    const crossedCount = affectedChildren.filter((child) =>
      Array.isArray(child.risk_codes) && child.risk_codes.includes("ABSENCE_LIMIT_EXCEEDED"),
    ).length;
  ...
  if (!riskFocus && incompleteChildren > 0) {
    // Incomplete means exactly one of check-in or check-out is logged.
  ...
  // Unscoped reviews stay county-first; narrowed reviews show child detail.
  const isNarrowedToDetail = isChildScoped || normalizedRequestedCounties.size > 0;
  // Pending-confirmation focus uses its dedicated two-table shape.
  const showGenericAttendanceLayout = !(isParentConfirmationsFocus && !isChildScoped);
  // Child-scoped all-risk reviews use separate tables per risk area.
  const isChildScopedMultiRisk = isChildScoped && !riskFocus;
  // The county-first pending block must not duplicate child-scoped rendering.
  if (isParentConfirmationsFocus && !isChildScoped && attendanceCounties.length > 0) {
    ...
    lines.push(
      ...
      // Risk children equals Children after focus filtering, so omit it.
      "| County | Children | Pending confirmations |",
      "| --- | ---: | ---: |",
      ...pendingCounties.map((county) => `| ${tableValue(county.county)} | ${tableValue(county.children)} | ${tableValue(county.pending_confirmation_days)} |`),
    );
  }
  if (isParentConfirmationsFocus && !isChildScoped && affectedChildren.length > 0) {
    ...
    lines.push(
      ...
      // #7 fix: blockquoted to match every other explanatory legend in this
      // file (payment-amounts legend, status legend) instead of a bare line.
      "> Days left = time remaining until the earliest deadline shown.",
      "| Child | Authorization | County | Pending | Earliest deadline | Days left |",
      "| --- | --- | --- | ---: | --- | ---: |",
      ...topPendingChildren.map((child) => `| ${tableValue(child.child_name)} | ${authorizationNames(child.authorization_names)} | ${tableValue(child.county)} | ${tableValue(child.pending_confirmation_days)} | ${typeof child.next_confirmation_deadline === "string" ? (shortDateLabel(child.next_confirmation_deadline) ?? tableValue(child.next_confirmation_deadline)) : "Unavailable from the current source"} | ${tableValue(child.confirmation_days_remaining)} |`),
    );
    if (affectedChildren.length > topPendingChildren.length) {
      // #5 fix: this is a global top-4 by pending count with no per-county
      // guarantee - disclose when at least one county with pending
      // confirmations isn't represented above, instead of implying full
      // county coverage.
      const representedCounties = new Set(topPendingChildren.map((child) => tableValue(child.county)));
      const countiesWithPending = new Set(
        attendanceCounties
          .filter((county) => numericValue(county.pending_confirmation_days) > 0)
          .map((county) => tableValue(county.county)),
      );
      const rankingNote = [...countiesWithPending].some((county) => !representedCounties.has(county))
        ? ", ranked by highest impact across all counties (not guaranteed per county)"
        : "";
      lines.push(
        "",
        `Showing the top ${topPendingChildren.length} of ${affectedChildren.length} affected children by pending confirmations${rankingNote}. Ask for a specific child by name for their full detail.`,
      );
    }
  }
  if (isChildScopedMultiRisk && affectedChildren.length > 0) {
    ...
    if (absenceRows.length > 0) {
      lines.push(
        ...
        "| --- | --- | --- | ---: | ---: |",
        ...absenceRows.map((child) => `| ${tableValue(child.child_name)} | ${authorizationNames(child.authorization_names)} | ${tableValue(child.county)} | ${tableValue(child.absence_days)} | ${tableValue(child.absence_limit)} |`),
      );
    }
    if (pendingRows.length > 0) {
      lines.push(
        ...
        // #7 fix: blockquoted for consistency with every other legend.
        "> Days left = time remaining until the earliest deadline shown.",
        "| Child | Authorization | County | Pending | Earliest deadline | Days left |",
        "| --- | --- | --- | ---: | --- | ---: |",
    ...
    if (incompleteRows.length > 0) {
      const incompleteDetailRows = incompleteRows.flatMap((child) => {
        const records = Array.isArray(child.incomplete_attendance_records)
          ? child.incomplete_attendance_records.map(recordValue).filter((record): record is Record<string, unknown> => Boolean(record))
          : [];
      ...
      lines.push(
        ...
        ...renderAttendanceNumericTable(["Child", "Authorization", "County", "Date", "Missing record", "Care hours at risk"], incompleteDetailRows, [5]),
      );
    }
  }
  if (affectedChildren.length > 0 && isNarrowedToDetail && showGenericAttendanceLayout && !isChildScopedMultiRisk) {
    ...
    const displayedChildren = isAbsenceLimitFocus
      ? sortedForDisplay.slice(
          (requestedDetailPage - 1) * requestedDetailPageSize,
          (requestedDetailPage - 1) * requestedDetailPageSize + requestedDetailPageSize,
        )
    ...
    // "align" lets a column override the default right-align (matches the two hardcoded pending-confirmation tables).
    const drillDownColumns: Array<{ label: string; value: (child: Record<string, unknown>) => string; present: boolean; align?: "---" | "---:" }> = isAbsenceLimitFocus
      ? [
      ...
      : isParentConfirmationsFocus
          ? [
            ...
            { label: "Earliest deadline", value: (child) => typeof child.next_confirmation_deadline === "string" ? (shortDateLabel(child.next_confirmation_deadline) ?? tableValue(child.next_confirmation_deadline)) : "Unavailable from the current source", present: displayedChildren.some((child) => typeof child.next_confirmation_deadline === "string"), align: "---" },
    ...
    if (isIncompleteAttendanceFocus) {
      ...
      lines.push(
        ...
        "Missing record identifies whether the check-in, check-out, or both records were not returned.",
        "",
        ...renderAttendanceNumericTable(["Child", "Authorization", "County", "Date", "Missing record", "Care hours at risk"], incompleteRows, [5]),
      );
    } else {
      lines.push(
        "",
    ...
    if (isAbsenceLimitFocus) {
          // Report the exact page range for complete affected-child pagination.
      ...
      if (affectedChildren.length > 0) {
        lines.push(
          "",
          `Showing children ${startRow}-${endRow} of ${affectedChildren.length} (page ${requestedDetailPage}; page size ${requestedDetailPageSize}).`,
        );
      }
  ...
  // Skip zero-risk overviews to avoid rendering an empty suppressed table.
  if (attendanceOverview && !isChildScoped && showGenericAttendanceLayout && affectedChildren.length > 0) {
    ...
    if (isIncompleteAttendanceFocus) {
      lines.push(
        "",
        "Attendance overview:",
        ...renderAttendanceNumericTable(
          ["Scheduled days", "Affected children", "Incomplete days", "Excluded holidays"],
          [[tableValue(attendanceOverview.scheduled_days), tableValue(attendanceOverview.affected_children), tableValue(incompleteDays), tableValue(attendanceOverview.excluded_holiday_dates)]],
          [0, 1, 2, 3],
        ),
      );
      ...
      lines.push(
        ...
        ...renderAttendanceNumericTable(
          ...
          [[tableValue(attendanceOverview.scheduled_days), tableValue(attendanceOverview.affected_children), tableValue(attendanceOverview.absence_days), tableValue(attendanceOverview.excluded_holiday_dates)]],
          [0, 1, 2, 3],
        ),
      );
    } else {
      lines.push(
        ...
        ...renderAttendanceNumericTable(
          ...
          [[tableValue(attendanceOverview.scheduled_days), tableValue(attendanceOverview.affected_children), tableValue(attendanceOverview.pending_confirmation_days), tableValue(attendanceOverview.absence_days), tableValue(attendanceOverview.excluded_holiday_dates)]],
          [0, 1, 2, 3, 4],
        ),
      );
    }
    if (numericValue(attendanceOverview.excluded_holiday_dates) > 0) {
      // Blockquoted with a leading blank line, matching every other footnote in this file -
      // previously a bare, unprefixed line directly under the table with no separation, which
      // read like a malformed extra table row.
      lines.push(
        "",
        "> Excluded holidays are paid separately as a holiday; not counted as an absence.",
      );
    }
  }
  ...
  if (attendanceCounties.length > 0 && showGenericAttendanceLayout && !isChildScopedMultiRisk) {
    if (riskFocus === "ABSENCE_LIMITS") {
      ...
      const topAbsenceCounties = [...attendanceCounties]
        .sort((left, right) =>
          numericValue(right.children_over_limit_count) - numericValue(left.children_over_limit_count)
          || numericValue(right.absence_days) - numericValue(left.absence_days),
        )
        .slice(0, 4);
      lines.push(
        "",
        // One status pattern per line - a "·"-joined single sentence was hard to scan and grew
        // denser every time a pattern was added.
        "> Over limit = already exceeded, real payment exclusion risk.",
        "> Approaching limit = within limit but close, no exclusion yet.",
        "> Within limit = no children over or approaching for this county.",
        "**Attendance by county (top counties by absence risk):**",
        ...renderAttendanceNumericTable(
          // #12 fix: "Risk children" dropped - always equals "Children" in
          // this riskFocus-scoped table (input already risk-filtered before
          // aggregation). #2 fix: "Children approaching limit" added -
          // childrenApproaching was already computed below but never
          // rendered, leaving the headline "N approaching" count
          // unreconcilable against this table.
          ...
          // Short label, not a full sentence - the legend above defines what "Over limit"/
          // "Approaching limit" mean, so the cell itself no longer needs to restate it in prose.
          const countyStatus = childrenOverLimit > 0
            ? `Over limit (${childrenOverLimit}/${totalChildren})`
            : childrenApproaching > 0
              ? `Approaching limit (${childrenApproaching}/${totalChildren})`
              : limitUnresolved
                ? "Limit unavailable"
                : "Within limit";
          ...
          return [tableValue(county.county), tableValue(county.children), tableValue(county.absence_days), approvedLimit, tableValue(childrenOverLimit), tableValue(childrenApproaching), countyStatus];
          }),
          [1, 2, 3, 4, 5],
        ),
      );
      ...
      // #12 fix: "Risk children" dropped - always equals "Children" here too.
      ...
      lines.push(
        "",
        "Attendance by county (top counties by incomplete attendance):",
        ...renderAttendanceNumericTable(
          ["County", "Children", "Incomplete days"],
          topIncompleteCounties.map((county) => [tableValue(county.county), tableValue(county.children), tableValue(county.incomplete_attendance_days)]),
          [1, 2],
        ),
      );
      ...
      lines.push(
        ...
        ...renderAttendanceNumericTable(
          ["County", "Children", "Pending confirmations"],
          topPendingCounties.map((county) => [tableValue(county.county), tableValue(county.children), tableValue(county.pending_confirmation_days)]),
          [1, 2],
        ),
      );
    } else {
  ...
  const rankedActions = Array.isArray(recordValue(analysis.situation)?.rankedActions) ? recordValue(analysis.situation)!.rankedActions as RankedNextAction[] : [];
  const baseActionIntents = actionMetadata(analysis.scope, risk, affectedChildren, false, typeof riskFocus === "string" ? riskFocus : undefined, rankedActions);
  ...
  const withPagination = absenceDetailPagination?.hasMore
    ...
    : withDrillDown;
  const recentSignatures = Array.isArray(analysis.recentActionSignatures) ? new Set(analysis.recentActionSignatures as string[]) : undefined;
  const actionIntents = withPagination
    .map((action) => actionViewMetadata(action, currentView))
    .filter((action) => !isNoOpAction(action, currentView) && !isRecentlyRenderedAction(action, recentSignatures));
...
export function authorizationNames(value: unknown): string {
...
function criticalityScore(row: Record<string, unknown>): number {
...
export function actionMetadata(
  ...
  riskFocus?: string,
  // Cross-capability, dollar/urgency-scored candidates from situation.rankedActions - replaces
  // the hardcoded per-risk-type pushes this function used to build itself. The ranking engine
  // (next_action_ranking.py) now owns "which risk is most worth reviewing right now"; this
  // function only maps its output into the existing action-object shape and still owns
  // view-navigation actions (drill-down/return/cross-capability links) below, which are
  // inherently tied to the current view, not risk scoring.
  ...
  // Suppress when already scoped to one child - avoids re-offering a dead-end action
  // (isNoOpAction in view-state.ts is a generic backstop, but riskFocus-narrowing inputs
  // don't always match a child-detail view's scope key-for-key, so this stays as a belt-
  // and-suspenders guard for the single-child case specifically).
  const isSingleChildDetailView = Array.isArray(scopeInput.childNames) && scopeInput.childNames.length === 1;
  if (!riskFocus && !isSingleChildDetailView) {
    for (const rankedAction of rankedActions) {
      actions.push({
        actionId: rankedAction.action_id,
        capability: rankedAction.tool === "cccap_analyze_payment" ? "payment-analysis" : "attendance-risk-analysis",
        tool: rankedAction.tool,
        label: rankedAction.label,
        reason: RANKED_ACTION_REASONS[rankedAction.action_id] ?? "Ranked by payment impact and urgency.",
        priority: rankedAction.priority_score,
        section: "next-actions",
        source: "current-result",
        scope,
        input: Object.assign({}, scopeInput, rankedAction.input),
      });
    }
  }
  ...
  const clearedScopeInput = (): Record<string, unknown> => {
    const cleared = { ...scopeInput };
    delete cleared.riskFocus;
    delete cleared.childNames;
    delete cleared.countyNames;
    delete cleared.detailPage;
    delete cleared.detailPageSize;
    return cleared;
  };
  if (riskFocus) {
    actions.push({
      ...
      section: "return",
      source: "current-result",
      input: clearedScopeInput(),
      scope,
    });
  }
  // Dead-end fallback: previously resubmitted the current scope verbatim (a guaranteed no-op
  // once nothing else applies, e.g. a single-child detail view with riskFocus already cleared),
  // which is exactly the loop chat_09_17_3.md reproduced. Now always resolves to a genuine state
  // change - returning to the unscoped summary - rather than repeating the current screen.
  if (actions.length === 0 && !includePaymentAction) {
    actions.push({
      ...
      tool: "cccap_analyze_payment_risk",
      label: "Return to attendance risk summary",
      reason: "No further action is available for this exact scope - go back to the full attendance summary.",
      priority: "medium",
      section: "next-actions",
      source: "current-result",
      input: clearedScopeInput(),
...
export function attendanceSummary(
  ...
  excludedClosureDates: string[],
  excludedHolidayDates: string[],
): Record<string, unknown> {
  const counties = new Map<string, Record<string, unknown>>();
  ...
  const scopeInput = recordValue(scope) ?? {};
  const availableViews: Record<string, unknown>[] = [
    {
  ...
  // Every child the evaluator returns carries at least one risk area by construction (this
  // view is only ever built from already risk-filtered children), so "any children present"
  // is the equivalent, schema-independent replacement for the old potential_impact string-
  // presence check (Python no longer emits provider-facing prose - see evaluate_attendance_risks.py).
...
// Internal - handles the greeting/facility-pulse snapshot shape (a
// top-level providerMessage string, from getAttendanceRiskSnapshot).
// Reached only through formatAttendanceResult() below.
function formatAttendanceSnapshot(data: unknown, includeContinuationMetadata = false): ToolResult {
  ...
  const snapshotRankedActions = Array.isArray(recordValue(snapshot.situation)?.rankedActions) ? recordValue(snapshot.situation)!.rankedActions as RankedNextAction[] : [];
  const actionIntents = risk
    ? actionMetadata(snapshot.scope, risk, children, true, undefined, snapshotRankedActions)
        .map((action) => actionViewMetadata(action, currentView))
        .filter((action) => !isNoOpAction(action, currentView))
    : [];
