...
function compactAttendanceSummary(
...
function compactActionControls(actions: Record<string, unknown>[]): Record<string, unknown>[] {
...
function estimatedMoney(value: unknown): string {
...
function hasAbsenceLimitConcern(child: Record<string, unknown>): boolean {
  ...
  function criticalityScore(row: Record<string, unknown>): number {
    ...
    let score = 0;
    if (codes.includes("ABSENCE_LIMIT_EXCEEDED")) score += 100;
    if (codes.includes("PARENT_CONFIRMATION_PENDING")) score += 80;
    if (codes.includes("INCOMPLETE_ATTENDANCE_RECORD")) score += 60;
...
function paymentSummaryView(value: unknown): Record<string, unknown> | undefined {
  return recordValue(value);
}

function compactPaymentSummaryView(value: Record<string, unknown> | undefined): Record<string, unknown> | undefined {
...
function summaryRows(value: unknown, key: string): Record<string, unknown>[] {
...
export function formatAttendanceRiskResult(data: unknown): ToolResult {
  ...
  if (attendanceOverview) {
    lines.push(
      ...
      "| ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
      `| ${tableValue(attendanceOverview.scheduled_days)} | ${tableValue(attendanceOverview.affected_children)} | ${tableValue(attendanceOverview.pending_confirmation_days)} | ${tableValue(attendanceOverview.absence_days)} | ${tableValue(attendanceOverview.incomplete_children)} | ${tableValue(attendanceOverview.excluded_closure_dates)} | ${tableValue(attendanceOverview.excluded_holiday_dates)} |`,
    );
  }
