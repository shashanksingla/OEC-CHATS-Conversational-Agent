...
import { CccapClient } from "./shared/transport.js";
import { ConversationContextStore, additiveRefinementsOnly, cacheKeyFor, scopeGranularity, type ConversationResultGraph, type ContinuationPlan, DialogueStateStore, conversationLogger as defaultConversationLogger, ConversationSessionStore, conversationSessionStore as defaultConversationSessionStore } from "./shared/conversation.js";
import {
...
import {
  ...
  result,
  type ToolResult,
} from './shared/formatters/shared.js';
...
function siblingCanonicalFacts(
  store: ConversationContextStore,
  providerKey: string,
  siblingTool: ContinuationPlan["tool"],
  scope: Record<string, unknown>,
  factsField: "attendanceRisk" | "payment",
): unknown {
  const cached = store.getCachedResult(cacheKeyFor(providerKey, siblingTool, scope), providerKey);
  if (!cached || cached.resultTool !== siblingTool) return undefined;
...
function contextualize(
  ...
  const providerMessage = value.content.find(
  ...
  const contextualizedValue: ToolResult = { ...value, structuredContent: structuredContentBase };
  const actions = Array.isArray(structuredContentBase.actionIntents)
      ? structuredContentBase.actionIntents.map(recordValue).filter((action): action is Record<string, unknown> => Boolean(action))
      : Array.isArray(structuredContentBase.actionControls)
        ...
        : [];
  if (actions.length === 0) return contextualizedValue;
  ...
  if (!hasPlanActions) return contextualizedValue;
  ...
  const graph: ConversationResultGraph = {
    ...
    ...(Array.isArray(structuredContentBase.responseSections)
      ? { availableEvidence: structuredContentBase.responseSections.filter((section): section is string => typeof section === "string") }
      : {}),
  ...
  const {
    ...
    daysUntilPayout: _daysUntilPayout,
    paymentDisclaimers: _paymentDisclaimers,
...
function filterSurfacedActions(data: unknown, providerKey: string, sessionStore: ConversationSessionStore): unknown {
  if (!data || typeof data !== "object" || Array.isArray(data)) return data;
  const situation = (data as Record<string, unknown>).situation;
  if (!situation || typeof situation !== "object" || Array.isArray(situation)) return data;
  const rankedActions = (situation as Record<string, unknown>).rankedActions;
  if (!Array.isArray(rankedActions)) return data;
  const unsurfaced = rankedActions.filter((action) => {
    const actionId = action && typeof action === "object" ? (action as Record<string, unknown>).action_id : undefined;
  ...
  for (const action of unsurfaced) {
    ...
    if (typeof actionId === "string") sessionStore.markActionSurfaced(providerKey, actionId);
  }
  const topAction = unsurfaced[0] as Record<string, unknown> | undefined;
  const { topPriorityActionId: _dropped, ...situationRest } = situation as Record<string, unknown>;
  return {
    ...data,
    situation: {
      ...situationRest,
...
function withGreeting(toolResult: ToolResult, providerDisplayName: string): ToolResult {
  const original = toolResult.content[0];
  if (!original || typeof original.text !== "string") return toolResult;
...
export function createServer(
  ...
  sessionStore: ConversationSessionStore = defaultConversationSessionStore,
): McpServer {
  ...
  const withConversationLogging = <T extends Record<string, unknown>>(
    ...
    let result = await handler(handlerInput as T);
    ...
    withConversationLogging("cccap_get_attendance_risk_snapshot", async (input) =>
      execute(
        ...
        (data) => attachDialogueState(
          contextualize(formatAttendanceResult(data, true), contextStore, providerKey, "continuation", data, "cccap_analyze_payment_risk"),
          dialogueStore,
          providerKey,
    ...
    withConversationLogging("cccap_analyze_payment_risk", async (input) => {
      ...
      if (!hasContinuation && request.riskFocus) {
        ...
        if (lastSnapshotScope && scopeGranularity(lastSnapshotScope) === "MONTH" && scopeGranularity(request) === "PERIOD") {
          ...
          const biasHint: "MONTH" | "PERIOD" = lastActiveScope && scopeGranularity(lastActiveScope as Record<string, unknown>) === "PERIOD" ? "PERIOD" : "MONTH";
          return formatScopeClarification(request.riskFocus, { dateFrom: request.dateFrom as string, dateTo: request.dateTo as string }, biasHint);
        }
      }
      ...
      if (cached && cached.resultTool === "cccap_analyze_payment_risk") {
        ...
        if (cachedResult) {
          ...
          return attachDialogueState(
            contextualize(
              formatAttendanceResult(continuationResult, true),
              contextStore,
              providerKey,
              "continuation",
              continuationResult,
      ...
      return execute(
        ...
        (data) => attachDialogueState(
          contextualize(formatAttendanceResult({ ...recordValue(data), recentActionSignatures: recentAttendanceSignatures }, true), contextStore, providerKey, "continuation", data, "cccap_analyze_payment_risk"),
          dialogueStore,
          providerKey,
          "attendance-risk-analysis",
          recordValue(data)?.scope,
    ...
    withConversationLogging("cccap_analyze_payment", async (input) => {
      ...
      if (cached && cached.resultTool === "cccap_analyze_payment") {
        ...
        if (cachedResult) {
          ...
          return attachDialogueState(
            contextualize(
              formatPayoutResult(continuationResult),
              contextStore,
              providerKey,
              "continuation",
              continuationResult,
      ...
      const runNextPayout = () => execute(
        ...
        (data) => attachDialogueState(
          contextualize(formatPayoutResult({ ...recordValue(data), recentActionSignatures: recentPaymentSignatures }), contextStore, providerKey, "continuation", data, "cccap_analyze_payment"),
          dialogueStore,
          providerKey,
          "payment-analysis",
      ...
      const runPaymentAnalysis = () => execute(
        ...
        () => getPaymentAnalysis(client, request, request.view, undefined, {
          ...
          ...(request.authNames ? { authNames: request.authNames } : {}),
          ...(request.countyNames ? { countyNames: request.countyNames } : {}),
        }, siblingAttendanceFacts),
        (data) => attachDialogueState(
          contextualize(formatPayoutResult(data), contextStore, providerKey, "continuation", data, "cccap_analyze_payment"),
          dialogueStore,
          providerKey,
          "payment-analysis",
          recordValue(data)?.scope,
