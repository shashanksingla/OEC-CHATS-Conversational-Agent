...
import { actionControls, countyPaymentSummary, estimatedMoney, plainMoney, humanizeAttendanceType, numericValue, recordValue, renderActionSections, result, shortDateLabel, tableValue, type ToolResult, DISCLAIMER_AT_RISK, DISCLAIMER_EXPECTED, DISCLAIMER_FORECASTED, DISCLAIMER_GLOBAL, DISCLAIMER_GUARANTEED, PAYMENT_AMOUNT_LEGEND } from './shared.js';
import { actionViewMetadata, viewState, type ProviderViewState } from '../view-state.js';
...
function formatPaymentAnalysisResult(data: unknown): ToolResult {
  ...
  if (status === "BLOCKED") {
    ...
    if (!isSettled && displayedNetPayment === 0 && Number.isFinite(displayAmountAtRisk) && displayAmountAtRisk > 0) {
      lines.push(
        "",
        `This shows ${plainMoney(0)} net because the payable amount is still conditional, not denied - an estimated ${plainMoney(displayAmountAtRisk)} remains possible once the pending confirmations below are completed. This is an estimate and will change as confirmations are completed.`,
...
export function renderCountyCategoryTable(rows: Record<string, unknown>[], estimatedTotal?: unknown, childrenServed?: unknown): string[] {
  const component = (row: Record<string, unknown>, key: string): number => {
  ...
  const totalRow = ["Total", childrenServed !== undefined ? String(Number(childrenServed) || 0) : String(rows.reduce((sum, row) => sum + money(row.children_served), 0)), ...(["enrollment_absence", "absence", "paid_holidays", "drop_in", "care", "vacant_slots", "risk"] as const).map((key) => {
