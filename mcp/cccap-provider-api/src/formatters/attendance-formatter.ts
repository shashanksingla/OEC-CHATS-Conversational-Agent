  ...
  if (attendanceCounties.length > 0 && !isChildScoped) {
    if (riskFocus === "ABSENCE_LIMITS") {
      ...
      lines.push(
        ...
        "| --- | ---: | ---: | ---: |",
        ...attendanceCounties.map((county) => `| ${tableValue(county.county)} | ${tableValue(county.children)} | ${tableValue(county.risk_children)} | ${tableValue(county.incomplete_attendance_days)} |`),
      );
    } else {
      lines.push(
        "",
