...
export function formatAttendanceRiskResult(data: unknown): ToolResult {
  ...
  if (affectedChildren.length > 0) {
    ...
    const drillDownColumns: Array<{ label: string; value: (child: Record<string, unknown>) => string; present: boolean }> = [
  ...
  if (attendanceOverview) {
    lines.push(
      "",
      "Attendance overview:",
      "| Scheduled days | Affected children | Pending confirmations | Absence days | Care days, including holidays | Excluded holidays |",
      "| ---: | ---: | ---: | ---: | ---: | ---: |",
      `| ${tableValue(attendanceOverview.scheduled_days)} | ${tableValue(attendanceOverview.affected_children)} | ${tableValue(attendanceOverview.pending_confirmation_days)} | ${tableValue(attendanceOverview.absence_days)} | ${tableValue(Math.max(0, numericValue(attendanceOverview.scheduled_days) - numericValue(attendanceOverview.excluded_closure_dates)))} | ${tableValue(attendanceOverview.excluded_holiday_dates)} |`, 
    );
  }
  if (attendanceCounties.length > 0) {
    if (riskFocus === "ABSENCE_LIMITS") {
      lines.push(
        ...
        "| County | Children | Children over limit | Status |",
        "| --- | ---: | ---: | --- |",
