import { compactActionControls, estimatedMoney, recordValue, renderActionSections, result, shortDateLabel, tableValue, type ToolResult, MAX_DISPLAY_CHILDREN, MAX_SUMMARY_ROWS } from './shared.js';
...
function compactAttendanceSummary(attendanceView: Record<string, unknown> | undefined): Record<string, unknown> | undefined {
...
export function formatAttendanceRiskResult(data: unknown, includeContinuationMetadata = false): ToolResult {
  ...
  if (isParentConfirmationsFocus && !isChildScoped && attendanceCounties.length > 0) {
    ...
    lines.push(
      ...
      "| --- | ---: | ---: | ---: |",
      ...pendingCounties.map((county) => `| ${tableValue(county.county)} | ${tableValue(county.children)} | ${tableValue(county.risk_children)} | ${tableValue(county.pending_confirmation_days)} |`),
    );
  }
  if (isParentConfirmationsFocus && !isChildScoped && affectedChildren.length > 0) {
    ...
    if (affectedChildren.length > topPendingChildren.length) {
      lines.push(
        "",
        `Showing the top ${topPendingChildren.length} of ${affectedChildren.length} affected children by pending confirmations. Ask for a specific child by name for their full detail.`,
      );
    }
  }
  ...
  if (attendanceCounties.length > 0 && showGenericAttendanceLayout && !isChildScopedMultiRisk) {
    if (riskFocus === "ABSENCE_LIMITS") {
      ...
      lines.push(
        ...
        ...renderAttendanceNumericTable(
          ["County", "Children", "Risk children", "Incomplete days"],
          topIncompleteCounties.map((county) => [tableValue(county.county), tableValue(county.children), tableValue(county.risk_children), tableValue(county.incomplete_attendance_days)]),
          [1, 2, 3],
        ),
      );
      ...
      lines.push(
        ...
        ...renderAttendanceNumericTable(
          ["County", "Children", "Risk children", "Pending confirmations"],
          topPendingCounties.map((county) => [tableValue(county.county), tableValue(county.children), tableValue(county.risk_children), tableValue(county.pending_confirmation_days)]),
          [1, 2, 3],
        ),
      );
    } else {
...
export function authorizationDates(value: unknown): string {
...
function overLimitDays(absenceDays: unknown, absenceLimit: unknown): string {
