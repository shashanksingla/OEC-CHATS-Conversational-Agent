import { DISCLAIMER_GLOBAL, estimatedMoney, renderActionSections, shortDateLabel, tableValue, type ToolResult } from "../shared/formatters/shared.js";
import type { PeriodComparisonResult, PeriodComparisonCategoryDelta } from "./payment-orchestration.js";

export function formatPeriodComparisonResult(data: unknown): ToolResult {
  const value = data as PeriodComparisonResult;
  if (!value || typeof value !== "object" || !value.periodOne || !value.periodTwo) return { content: [{ type: "text", text: JSON.stringify(data) }] };
  ...
  const lead = notable.length > 0
    ? `You're expected to receive ${estimatedMoney(Math.abs(delta))} ${delta >= 0 ? "more" : "less"} this period than last, mainly due to ${tableValue((notable[0] as PeriodComparisonCategoryDelta).label)}.`
