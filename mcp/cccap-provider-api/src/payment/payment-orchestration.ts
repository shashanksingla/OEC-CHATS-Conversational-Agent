...
import { CccapClient, type DateScope } from "../shared/transport.js";
import {
...
function highestImpactChildName(impacts: unknown[], days: unknown[]): HighestImpactChild {
  ...
  // Prefer total dollar exposure before falling back to scheduled hours when risk is zero.
  ...
  // Use scheduled hours only when no verified dollar amount is available.
...
export function scopeAuthorizationDataToSchedules(
  ...
  // Keep a single already-validated record when the period has schedules; never broaden an unresolved mapping.
...
// Ledger views are routed separately but remain in this union for schema consistency.
...
export async function getPaymentAnalysis(
  ...
  // Optional known period ID and release date preserve authoritative ledger values for CUSTOM_RANGE calls.
  filters: { childNames?: string[]; authNames?: string[]; countyNames?: string[]; knownServicePeriodId?: string; knownPaymentReleaseDate?: string } = {},
  // Sibling attendance capability's last-known facts (from the result cache, not a new fetch) - lets action ranking see both capabilities together.
  siblingAttendanceFacts?: unknown,
): Promise<unknown> {
  const initialization = record(await client.initialize(scope), "Provider context");
  // CUSTOM_RANGE uses its requested span without selecting a Salesforce service period.
  ...
  // Non-STATUS views use the selected service period's date range.
  ...
  // Push explicit authorization filters into schedule retrieval to keep downstream derivations scoped.
  ...
  // Collect every rate type used by an authorization because it may vary by day.
  ...
  // Opt-in ledger diagnostic logs the requested range and evaluator attendance span.
  ...
  try {
    ...
    const displayableDays = allDays.filter((value) => {
      ...
      // Keep rows eligible for the "review excluded payment days" action.
      return true;
    });
    ...
    // Return a bounded preview without full-page detail mode.
    ...
    return {
      ...
      sourceRetrievedAt,
      situation: await buildSituationEnvelope(result.payment, "payment-analysis", sourceScope, sourceRetrievedAt, siblingAttendanceFacts),
    };
  } finally { await rm(directory, { recursive: true, force: true }); }
}

// OVERDUE marks an unpaid period past both confirmation and payout deadlines.
export type LedgerPeriodStatus = "NOT_YET_STARTED" | "IN_PROGRESS" | "PENDING_CONFIRMATION" | "EXPECTED_AWAITING_PAYOUT" | "OVERDUE" | "PAID";
// PAID periods expose netAmount; unreleased periods expose calculatedAmount, with summary breakdowns preserved.
...
export async function getServicePeriodLedger(client: CccapClient, scope: DateScope, asOfDate: string, options: { periodCount?: number; unbounded?: boolean; filters?: PaymentLedgerFilters; isolateFailures?: boolean } = {}): Promise<{ periods: LedgerPeriodEntry[]; sourceRetrievedAt: string }> {
  // Multi-period views isolate failed periods; NEXT_PAYOUT evaluates exactly one candidate strictly.
  const count = options.periodCount ?? (options.unbounded ? 1 : 5);
  // Strict NEXT_PAYOUT propagation prevents a failed sole candidate from becoming a false "no payout" result.
  const isolateFailures = options.isolateFailures ?? true;
  await client.initialize();
  // unbounded (getUpcomingPayoutDetail): finds the soonest unpaid period via paymentAfter:"TODAY" with no service-date lower bound, so a period starting in a prior month is never missed.
  ...
  // Use server-side limitOne when exactly one period is requested.
  const response = record(await client.getServicePeriods({ ...ledgerScope, limitOne: count === 1 }), "Service periods");
  // Preserve the source payout date so ledger entries reflect the authoritative release date.
  ...
  // Preserve authoritative service-period IDs and isolate evaluation failures per period.
  const settledResults = await Promise.all(selected.map(async (period) => {
    try {
      ...
      if (!isolateFailures) {
        // Strict single-period calls must propagate the period-scoped error.
  ...
  // Prefer authoritative release dates, then engine output, then the local fallback.
  const periods = results.map(({ period, result }) => { const payment = record(result.payment, "Evaluated payment"); const payoutDate = period.paymentReleaseDate ?? (typeof payment.payout_date === "string" ? payment.payout_date : computePayoutDate(period.serviceEndDate));
    // Deliberately disable duplicate-payment settlement until the NEXT_PAYOUT behavior is revisited.
    ...
    const settledAmount = typeof payment.settled_amount === "string" || typeof payment.settled_amount === "number"
      ...
      : undefined;
    // Classify future periods and overdue periods using begin date, confirmation window, and payout date.
    ...
    // Use the settled amount for PAID periods, falling back to the calculated amount.
    ...
    return {
      ...
      countyComposition,
      categories,
      // Expose settled amounts as Net and unreleased amounts as Calculated.
...
// Next payout selects the soonest unpaid/upcoming release date.
export async function getUpcomingPayoutDetail(client: CccapClient, scope: DateScope, asOfDate: string, filters?: PaymentLedgerFilters): Promise<{ entry: LedgerPeriodEntry | undefined; daysUntilPayout: number | undefined; sourceRetrievedAt: string }> {
  // Search unbounded and propagate errors so NEXT_PAYOUT evaluates exactly one real candidate.
  const ledger = await getServicePeriodLedger(client, scope, asOfDate, {
    unbounded: true,
    isolateFailures: false,
    ...(filters ? { filters } : {}),
  });
  // Exclude paid, overdue, and already-released periods from NEXT_PAYOUT.
...
// Last payout selects the most recently released PAID period on explicit request.
