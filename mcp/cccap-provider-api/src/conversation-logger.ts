...
export class ConversationLogger {
  ...
  logToolCall(entry: {
    ...
    durationMs: number;
    // The provider's exact verbatim chat message/selection for this turn
    // (e.g. "3", "Review absence-limit risk"), threaded in separately from
    // `input` by server.ts's withConversationLogging wrapper - never part
    // of the structured tool-call arguments the real handler sees. Logged
    // ahead of `Input:` so a reviewer can see what the provider actually
    // said/picked, not just what request that produced.
