...
export type ContinuationPlan = {
  ...
  // Internal link to the matching sessionActions record (by actionId), used
  // only so resolve() can mark that record SELECTED, mirroring what
  // resolveAction() already does for the actionId-based continuation path.
  // Always stripped by publicPlan() before a plan is returned to a caller.
  _selectionActionId?: string;
};
