...
function relationshipRecords(value: unknown): unknown[] {
  if (!value || typeof value !== "object" || Array.isArray(value)) return [];
  const related = (value as RecordValue).records;
...
function date(value: unknown, label: string): string {
  ...
  if (!result) throw new Error(`${label} is unavailable`);
  return result;
}

export function normalizeProviderInitialization(value: unknown): RecordValue {
  ...
  const fiscalAgreements = records(source.fiscalAgreements, "Fiscal agreements").map((agreement) => ({
    ...
    county_name: optionalString((agreement.CDE_COUNTY__r && typeof agreement.CDE_COUNTY__r === "object" && !Array.isArray(agreement.CDE_COUNTY__r))
      ? (agreement.CDE_COUNTY__r as RecordValue).Name
      : undefined),
...
export function normalizeCases(value: unknown): RecordValue {
...
export function normalizeAuthorizations(value: unknown): RecordValue {
...
export function normalizeSchedules(value: unknown): RecordValue {
  const source = record(value, "Schedules");
  const normalized = normalizeScheduleAttendance(
    records(source.schedules, "Schedules"),
  );
  return {
...
export function normalizeFiscalRates(value: unknown): RecordValue {
...
export function normalizeCountyPlans(value: unknown): RecordValue {
...
export function normalizeServicePeriods(value: unknown): RecordValue {
...
export function normalizeHolidays(value: unknown): RecordValue {
...
export function normalizePaymentHistory(value: unknown): RecordValue {
