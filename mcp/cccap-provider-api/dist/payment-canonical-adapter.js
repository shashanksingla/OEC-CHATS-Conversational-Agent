...
export function normalizePaymentSourceBundle(sources) {
    ...
    const authorizations = normalizedAuthorizations.map((value, index) => {
        ...
        if (typeof authorization.Id !== "string" ||
            ...
            throw new Error(`Authorization fiscal schedule mapping failed for row ${index}`
    ...
    const payload = buildCanonicalPaymentPayload({
        ...
        feeSchedules,
        vacantSlotSchedules: normalizeVacantSlotSchedules(vacantSlotData.vacantSlots, normalizedFiscal.fiscalRates, countyIds, providerTier, sources.initialization),
...
export function normalizeVacantSlotSchedules(slots, normalizedFiscalRates, countyIds, providerQualityTier, initialization) {
