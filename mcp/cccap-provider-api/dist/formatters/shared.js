...
export function renderCountyComposition(rows, options = {}) {
    const component = (row, key) => recordValue(row[key]) ?? (key === "attendance_based" || key === "authorized_based"
        ? { amount: row[`${key}_amount`] }
        : {});
    const countyRiskAmount = (row) => {
        ...
        return components.length > 0
            ? components.reduce((total, value) => total + (Number(value) || 0), 0)
            : undefined;
    };
    const headers = [
        "County",
    ...
    const sumComponent = (key) => rows.reduce((total, row) => {
