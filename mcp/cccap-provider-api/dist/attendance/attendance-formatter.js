...
import { actionControls, attendanceScopeLabel, compactActionControls, estimatedMoney, hasAbsenceLimitConcern, numericValue, overLimitDays, recordValue, renderActionSections, result, shortDateLabel, tableValue, MAX_DISPLAY_CHILDREN, MAX_SUMMARY_ROWS } from '../shared/formatters/shared.js';
 ...
 */
export function formatAttendanceResult(data, includeContinuationMetadata = false) {
...
function formatAttendanceRiskAnalysis(data, includeContinuationMetadata = false) {
    ...
    if (isParentConfirmationsFocus && !isChildScoped && attendanceCounties.length > 0) {
        const pendingCounties = [...attendanceCounties]
            ...
            .slice(0, 4);
        lines.push("", "Attendance by county (pending confirmations):", 
        // Risk children equals Children after focus filtering, so omit it.
        "| County | Children | Pending confirmations |", "| --- | ---: | ---: |", ...pendingCounties.map((county) => `| ${tableValue(county.county)} | ${tableValue(county.children)} | ${tableValue(county.pending_confirmation_days)} |`));
    }
    if (isParentConfirmationsFocus && !isChildScoped && affectedChildren.length > 0) {
        ...
        if (affectedChildren.length > topPendingChildren.length) {
            ...
            const countiesWithPending = new Set(attendanceCounties
            ...
            const rankingNote = [...countiesWithPending].some((county) => !representedCounties.has(county))
                ...
                : "";
            lines.push("", `Showing the top ${topPendingChildren.length} of ${affectedChildren.length} affected children by pending confirmations${rankingNote}. Ask for a specific child by name for their full detail.`);
        }
    }
    ...
    if (attendanceOverview && !isChildScoped && showGenericAttendanceLayout && affectedChildren.length > 0) {
        ...
        if (numericValue(attendanceOverview.excluded_holiday_dates) > 0) {
            ...
            lines.push("", "> Excluded holidays are paid separately as a holiday; not counted as an absence.");
        }
    }
    ...
    if (attendanceCounties.length > 0 && showGenericAttendanceLayout && !isChildScopedMultiRisk) {
        if (riskFocus === "ABSENCE_LIMITS") {
            ...
            "> Over limit = already exceeded, real payment exclusion risk.", "> Approaching limit = within limit but close, no exclusion yet.", "> Within limit = no children over or approaching for this county.", "**Attendance by county (top counties by absence risk):**", ...renderAttendanceNumericTable(
            ...
            ["County", "Children", "Absence days", "Monthly absence limit", "Children over limit", "Children approaching limit", "Status"], topAbsenceCounties.map((county) => {
            ...
            const topIncompleteCounties = [...attendanceCounties]
                ...
                .slice(0, 4);
            lines.push("", "Attendance by county (top counties by incomplete attendance):", ...renderAttendanceNumericTable(["County", "Children", "Incomplete days"], topIncompleteCounties.map((county) => [tableValue(county.county), tableValue(county.children), tableValue(county.incomplete_attendance_days)]), [1, 2]));
        }
            ...
            const topPendingCounties = [...attendanceCounties]
                .sort((left, right) => numericValue(right.pending_confirmation_days) - numericValue(left.pending_confirmation_days))
                .slice(0, 4);
            lines.push("", "Attendance by county (pending confirmations):", ...renderAttendanceNumericTable(["County", "Children", "Pending confirmations"], topPendingCounties.map((county) => [tableValue(county.county), tableValue(county.children), tableValue(county.pending_confirmation_days)]), [1, 2]));
        }
        else {
    ...
    const rankedActions = Array.isArray(recordValue(analysis.situation)?.rankedActions) ? recordValue(analysis.situation).rankedActions : [];
    ...
    const withPagination = absenceDetailPagination?.hasMore
        ...
        : withDrillDown;
    const recentSignatures = Array.isArray(analysis.recentActionSignatures) ? new Set(analysis.recentActionSignatures) : undefined;
...
function formatAttendanceSnapshot(data, includeContinuationMetadata = false) {
    ...
    const snapshotRankedActions = Array.isArray(recordValue(snapshot.situation)?.rankedActions) ? recordValue(snapshot.situation).rankedActions : [];
