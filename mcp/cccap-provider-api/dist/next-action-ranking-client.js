 ...
 */
export async function rankNextActions(canonicalFacts, activeCapability) {
    const directory = await mkdtemp(join(tmpdir(), "carepay-ranking-"));
    const inputPath = join(directory, "ranking.json");
    try {
        await writeFile(inputPath, JSON.stringify({ canonical_facts: canonicalFacts, active_capability: activeCapability }), "utf8");
