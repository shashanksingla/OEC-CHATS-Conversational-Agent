...
Fiscal-rate normalization maps Salesforce age-group codes `1`-`8`, care-unit codes `1`-`5`, and the canonical rate-type codes (`1`, `13`, `19`, `25`, `31`, `37`, `43`, `55`, `91`) to their Salesforce labels. The R00393 lookup is also exposed as `1 -> 15650` and `ADD -> 5500`. Authorization responses include a fail-closed fiscal-schedule match using the schedule's `CI_Authorization_Rate_Type__c`, provider scope, county, authorization dates, and requested `careDate`; slot-contract rate type is an optional consistency check. Vacant and occupied slot contracts remain separate occupancy/fee context; they do not select the attendance fiscal schedule. Ties use the latest schedule start date, while no-match and conflicting results remain unresolved.
